...
def dnn_model_kfold(k=10):
    ...
    """
    samples = csv2dict("C:/Users/Admin/Desktop/bug-localization-by-dnn-and-rvsm-master/data/features.csv")
    ...
    for key in acc_dicts[0].keys():
        ...
        print(f"Average {key} Accuracy: {avg_acc_dict[key]}")  # Print average accuracy

        # Print accuracy of each fold
        for i, acc in enumerate([d[key] for d in acc_dicts]):
            print(f"Fold {i+1} {key} Accuracy: {acc}")
